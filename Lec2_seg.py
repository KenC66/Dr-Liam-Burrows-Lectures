...
def train(num_epochs, model, train_loader):
    ...
    print('\ttrain [ %s ] for %d epochs, %d sets of data  each with batch size = %d' % 
          (str(model).split('(')[0],num_epochs,total_step,train_loader.batch_size))    
    for epoch in range(num_epochs):
        ...
        timea = time.time() - time0; timen=timea*(num_epochs-1-epoch)           
...
if __name__ == '__main__':  
    ...
    print(image.size(),label.size(),label.size())


    # In[ ]: UNet = UNet() ; x = torch.zeros([1,3,256,256]); y = UNet(x)
    ...
    learning_rate = 0.001;  s24=1014**2
