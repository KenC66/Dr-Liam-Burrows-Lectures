...
def train(num_epochs, model, loaders):
    ...
    print('\ttrain [ %s ] for %d epochs, %d sets of data  each with batch size = %d' % 
          (str(model).split('(')[0],num_epochs,total_step,loaders['train'].batch_size))    
...
if __name__ == '__main__':    
    ...
    print(total_step,  '==>',  train_imgs.shape)

    sfile = 'model_sav_den_%s.h5' % (str(model).split('(')[0])
